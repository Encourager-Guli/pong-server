...
class StableBaselinesGodotEnv(VecEnv):
    ...
    def step(self, action: np.ndarray) -> Tuple[Dict[str, np.ndarray], np.ndarray, np.ndarray, List[Dict[str, Any]]]:
        ...
        action2= self.opponent.predict(self.process_obs(self.obs), deterministic=True)
        # print(self.obs,'\n',self.process_obs(self.obs))
        # Send actions to each environment
        for i in range(self.n_parallel):
            self.envs[i].step_send(action[i * num_envs : (i + 1) * num_envs],action2[i * num_envs : (i + 1) * num_envs])
        # print(self.obs)
        # Receive results from each environment
        for i in range(self.n_parallel):
            obs, reward, term, trunc, info = self.envs[i].step_recv()
            if term == [True]:
                self.opponent.refresh_policy(i)
            all_obs.extend(obs)
    ...
    # 将观测到的全局值切换到对应玩家视角空间下的观测值
    def process_obs(self,obs):
        new_obs = {
            "obs": [row.copy() for row in obs["obs"]]
        }
        for i in range(self.n_parallel):
            new_obs["obs"][i][0] = 1 - new_obs["obs"][i][0]
            new_obs["obs"][i][1] = 1 - new_obs["obs"][i][1]
            new_obs["obs"][i][2] *= -1
            new_obs["obs"][i][3] = 1 - new_obs["obs"][i][3]
            new_obs["obs"][i][4] = 1 - new_obs["obs"][i][4]
            new_obs["obs"][i][5] *= -1
            new_obs["obs"][i][6] = 1 - new_obs["obs"][i][6]
            new_obs["obs"][i][7] = 1 - new_obs["obs"][i][7]
            new_obs["obs"][i][8] *= -1
            new_obs["obs"][i][9] *= -1
            new_obs["obs"][i][0], new_obs["obs"][i][3] = new_obs["obs"][i][3], new_obs["obs"][i][0]  # 交换第 1 和第 4 个元素
            new_obs["obs"][i][1], new_obs["obs"][i][4] = new_obs["obs"][i][4], new_obs["obs"][i][1]  # 交换第 2 和第 5 个元素
            new_obs["obs"][i][2], new_obs["obs"][i][5] = new_obs["obs"][i][5], new_obs["obs"][i][2]  # 交换第 3 和第 6 个元素
        return new_obs
