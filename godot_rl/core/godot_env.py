...
class GodotEnv:
    ...
    def step_send(self, action,action2=None, order_ij=False):
        """
        ...
        if action2 is not None:
            ...
            action2 = self.action_space_processor.to_original_dist(action2)
            message = {
                "type": "action",
                "action": self.from_numpy(action, order_ij=order_ij),
                "action2": self.from_numpy(action2, order_ij=order_ij),
            }
