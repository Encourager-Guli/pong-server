...
# 2.  chmod +x servers/godot_rl_BallChase/bin/BallChase.x86_64
if can_import("ray"):
...
opponent_model = PPO.load(path_zip, env=env)
opponent_pool=OpponentPool(opponent_model,args.n_parallel)

env = VecMonitor(env)
env.venv.opponent=opponent_pool
...
if args.resume_model_path is None:
    ...
    model: PPO = PPO(
        ...
        n_steps=512,
        policy_kwargs={
            # 关键修改：pi 和 vf 均设置为 4 层，每层 64 神经元
            "net_arch": [dict(pi=[64, 256, 256, 64], vf=[64, 256, 256, 64])]
        },
